...
=item * B<update_lastseen_for_articlecopies> - If a library is tracking the last
activity of patrons, a patron might be actively requesting article copies thorugh
ILL, and that activitiy will not be recorded anywhere. By enabling this config
variable, the "lastseen" property of a patron will be updated when an article
ILL is received for that patron.
